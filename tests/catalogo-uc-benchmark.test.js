...
console.log('\n=== Resultado de Fase 3 sobre la línea base congelada ===');
const result=benchmark.ejecutarBenchmark();
chk('el parser procesa las 30 fuentes sin red',result.rows.length===30);
chk('la exactitud de clasificación mejora a 30 de 30',result.metrics.exactClassificationCount===30);
chk('los pesos explícitos calzan en 30 de 30',result.metrics.exactWeightsCount===30);
chk('no hay falsos auto-importable',result.metrics.falseAutoImportableCount===0&&result.metrics.falseAutoImportableRate===0);
chk('ya no quedan falsos insufficient_information',result.metrics.falseInsufficientInformationCount===0);
chk('se detectan las 15 señales complejas humanas',result.metrics.complexRuleSignalCount===15&&result.metrics.missedComplexRuleSignalCount===0);
...
chk('QIM100F detecta eximición y fórmula alternativa',(()=>{const r=result.rows.find(x=>x.courseCode==='QIM100F');return r.missedComplexRules.length===0&&r.parserReasons.includes('exemption_rule_detected')&&r.parserReasons.includes('alternative_final_grade_formula_detected');})());
