...
function setSlotNota(catId,slot,raw){
  ...
  if(txt)showToast(lecturaDespuesDeNota(r));
}
...
function openModal(){
  ...
  sheet.ontouchmove=e=>{
    ...
    if(dy<=0){sheet.style.transform='';return;} // hacia arriba → permitir scroll interno
...
// Traduce una nota recién ingresada a una consecuencia académica concreta.
// No promete aprobación: siempre aclara que el margen se calcula sobre lo que
// falta por rendir y respeta compuertas ya activadas.
